...
getgenv().mouse = plrs.LocalPlayer:GetMouse() 
-- UI Variables
local Remove,Insert,Find = table.remove,table.insert,table.find 
...
getgenv().library = {
    ...
    watermarktext = "gsz1 | {fps} | {game} | {name} | {ping} | {date}"
}
--
...
local utility = {
    Keyboard = {
        ...
        InputNames = {
            ...
            Insert = "Ins", NumLock = "NumL", Comma = ",", Period = ".",Space = "Space"
        }
    }
}
...
do
	...
	do
            ...
            do -- Preview Stuff
                ...
                --
                local preview_titlee = utility:Create("TextLabel", {Vector2.new(preview_box.Size.X / 2, 0), preview_box}, {
                    Text = "Username",
                    Size = theme.textsize,
                    Font = theme.font,
                    Color = theme.textcolor,
                    OutlineColor = theme.textborder,
                    Center = true,
                    Position = utility:Position(0.5, 0, 0, 0, preview_box)
                }, window.VisualPreview.Drawings)
                --
                local preview_titleee = utility:Create("TextLabel", {Vector2.new(preview_box.Size.X / 2, 20), preview_box}, {
                    Text = "Username",
                    Size = theme.textsize,
                    Font = theme.font,
                    Color = theme.textcolor,
            ...
            do -- New Drawings
                ...
                for i,v in next, window.VisualPreview.Drawings do 
                    if v.Size == theme.textsize then 
                        if v.Text == "Username" then 
                            table.insert(getgenv().Stats,v)
                        end     
                    end
                end 
                --
	...
	-- 
        ...
        function window:Watermark(info)
            ...
            --
            function window.watermark:UpdateText(text)
                watermark_title.Text = text
                :gsub("{game}", game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId).Name)
                :gsub("{name}", game.Players.LocalPlayer.Name)
                :gsub("{fps}", "FPS: " .. library.shared.fps)
                :gsub("{date}", os.date("%b. %d, %Y"))
                :gsub("{ping}", "Ping: " ..library.shared.ping)
            end
            --
            ...
            utility:Connection(rs.RenderStepped, function(FPS)
                ...
                if (tick() - Tick) > 0.15 then
                    window.watermark:UpdateText(library.watermarktext)
                    window.watermark:UpdateSize()
                    --
                    Tick = tick()
                end
        ...
        function window:NotificationList(info)
            ...
            function window.notificationlist:AddNotification(info)
                ...
                local notify_outline = utility:Create("Frame", {Vector2.new(0, 0)}, {
                    Size = utility:Size(0, 18 + (7 * #text), 0, 19),
                    Position = utility:Position(0, 29, 0, 34 + (#window.notificationlist.notifications * 20)),
                    Color = theme.outline,
                    ZIndex = 1,
                    Transparency = 0,
                    Hidden = true,
                ...
                local notify_title = utility:Create("TextLabel", {Vector2.new(8, 5), notify_frame}, {
                    Size = 13,
                    Position = utility:Position(0, 10, 0, 1, notify_frame),
                    Font = Drawing.Fonts.Plex,
                    Color = theme.textcolor,
                    Outline = true,
                    ZIndex = 5,
                ...
                --
                utility:LoadImage(notify__gradient, "gradient", "https://raw.githubusercontent.com/f1nobe7650/other/main/uis/osiris/gradient.png")
                --
            ...
            function window.notificationlist:SortNotifications()
                for index, notification in next, window.notificationlist.notifications do
                    notification["notify_outline"].Position = utility:Position(0, 0, 0, 0, notification["notify_outline"])
                    ...
                    notification["notify_title"].Position = utility:Position(0, 8, 0, 1, notification["notify_frame"])
    ...
    function pages:MultiSection(info)
        ...
        function multiSection:Update(Padding)
            ...
            print(multiSection_inline.Size)
...
function pages:PlayerList(info)
        ...
        for Index = 1, 10 do
            ...
            --
            function item:set(enabled, selected) -- fix 1
    ...
    function sections:Dropdown(info)
        ...
        local section = self
        --
        local dropdown = {open = false, scrollindex = max and 0, scrolling = max and {true, nil}, current = tostring(def), options = options, holder = {buttons = {}, drawings = {}}, axis = section.currentAxis}
        --
    ...
    function sections:Listbox(info)
        ...
        --
        utility:LoadImage(listbox_gradient, "gradient", "https://raw.githubusercontent.com/f1nobe7650/other/main/uis/osiris/gradient.png")
        --
    ...
    function sections:ConfigBox(info)
        ...
        --
        utility:LoadImage(configLoader_gradient, "gradient", "https://raw.githubusercontent.com/f1nobe7650/other/main/uis/osiris/gradient.png")
        --
